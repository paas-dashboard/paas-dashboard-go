...
func pulsarConsumerAbnormalCheck(instance *module.PulsarInstance) {
	...
	tk := toolbox.NewTask("abnormalCheck", cronExpress, func() error {
		...
		hosts, err := GetHosts()
		if err != nil {
			logrus.Errorf("abnormal get host failed: %v", err)
			return err
		}
		...
		for _, host := range hosts {
			...
			if err != nil {
				logrus.Errorf("connect pulsar failed: %v", err)
				continue
			}
	...
	err := tk.Run()
	if err != nil {
		logrus.Errorf("task exec failed: %v", err)
		return
	}
...
func checkConsumerAbnormalInterval(admins []*padmin.PulsarAdmin) (int, int) {
	...
	for _, tenant := range tenantList {
		...
		if err != nil {
			logrus.Errorf("check consumer abnormal|get namespace by tenant %s failed: %v", tenant, err)
			continue
		}
		for _, namespace := range namespaceList {
			...
			if err != nil {
				logrus.Errorf("check consumer abnormal|get topiclist by tenant %s and namespace %s failed: %v", tenant, ns, err)
				continue
			}
